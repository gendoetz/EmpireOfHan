...
int pc_addtocurrent2(struct block_list *bl, va_list ap) {
	...
	if(fl->data.id==fl2->data.id && fl->data.owner_id == fl2->data.owner_id && fl->data.protectnum == fl2->data.protectnum && !(strcmpi(fl->data.real_name, fl2->data.real_name))) {
...
int pc_givexp(USER *sd, unsigned int exp, unsigned int xprate) {
	...
	//adding in totem times

	//Chung Ryong
	if(cur_time>3 && cur_time<10) {
		if(sd->status.totem == 3) {
			totem_time = 1;
		}
	}
	//Jujak
	if(cur_time>9 && cur_time<16) {
		if(sd->status.totem == 0) {
			totem_time = 1;
		}
	}
	//Baekho
	if(cur_time>15 && cur_time<22) {
		if(sd->status.totem == 1) {
			totem_time = 1;
		}
	}
	//Hyun moo
	if(cur_time>21 && cur_time<4) {
		if(sd->status.totem == 2) {
			totem_time = 1;
		}
	}
	...
	if(totem_time == 1) {
		...
		len=sprintf(xpstring,"%u experience! (T)",defaultxp);
...
int pc_calcstat(USER *sd) {
	...
	//Commented out these lines because adding % to main hp/mp is too complex with item values
...
int pc_isinvenspace(USER *sd, int id, int owner, int protectnum, char *engrave) {
	int i;
	int uniqueamount = 0;
	
	if (itemdb_unique(id) > 0) {
		for (i = 0; i < sd->status.maxinv; i++) {
			if(sd->status.inventory[i].id == id && itemdb_unique(id) > 0 && sd->status.inventory[i].owner_id == owner && sd->status.inventory[i].protectnum == protectnum && !(strcmpi(sd->status.inventory[i].real_name, engrave))) {
				uniqueamount += sd->status.inventory[i].amount;
			}
		}
	
		for (i = 0; i < 14; i++) {
			if (sd->status.equip[i].id == id && itemdb_unique(id) > 0 && sd->status.inventory[i].owner_id == owner && sd->status.inventory[i].protectnum == protectnum && !(strcmpi(sd->status.inventory[i].real_name, engrave))) {
				uniqueamount += 1;
			}
		}
		
		for (i = 0; i < sd->status.maxinv; i++) {
			if (sd->status.inventory[i].id == id && sd->status.inventory[i].amount < itemdb_maxamount(id) && uniqueamount < itemdb_unique(id) && sd->status.inventory[i].owner_id == owner && sd->status.inventory[i].protectnum == protectnum && !(strcmpi(sd->status.inventory[i].real_name, engrave)))
				return i;
		}
		
		for (i = 0; i < sd->status.maxinv; i++) {
			if (sd->status.inventory[i].id == 0 && uniqueamount < itemdb_unique(id) && sd->status.inventory[i].owner_id == owner && sd->status.inventory[i].protectnum == protectnum && !(strcmpi(sd->status.inventory[i].real_name, engrave)))
				return i;
		}
		...
		for (i = 0; i < sd->status.maxinv; i++) {
			if (sd->status.inventory[i].id == id && sd->status.inventory[i].amount < itemdb_maxamount(id) && sd->status.inventory[i].owner_id == owner && sd->status.inventory[i].protectnum == protectnum && !(strcmpi(sd->status.inventory[i].real_name, engrave)))
				return i;
		}
...
int pc_isinvenspace_unequip(USER *sd, int id, int owner, int protectnum, char *engrave) {
	int i;
...
int pc_isinvenitemspace(USER *sd, int num, int id, int owner, int protectnum, char *engrave) {
	int i;
	int uniqueamount = 0;
	char msg[255];
...
int pc_additem(USER *sd, struct item *fl) {
	...
	if ((num = pc_isinvenspace(sd, fl->id, fl->owner_id, fl->protectnum, fl->real_name)) >= sd->status.maxinv) {
	...
	do {
		if (fl->amount > (i = pc_isinvenitemspace(sd, num, fl->id, fl->owner_id, fl->protectnum, fl->real_name))) {
			...
			sd->status.inventory[num].protectnum = fl->protectnum;
		...
		num = pc_isinvenspace(sd, fl->id, fl->owner_id, fl->protectnum, fl->real_name);
...
int pc_additem_unequip(USER *sd, struct item *fl) {
	char RegStr[] = "goldbardupe";
	char msg[255];
	int num;
	int i;
	...
	if ((num = pc_isinvenspace_unequip(sd, fl->id, fl->owner_id, fl->protectnum, fl->real_name)) >= sd->status.maxinv) {
			clif_sendminitext(sd, map_msg[MAP_ERRITMFULL].message, map_msg[MAP_ERRITMFULL].len);
			pc_dropitemfull(sd, fl);
		return 0; //Used to be 1
	}
	...
	do {
		...
		clif_sendadditem(sd, num);
		num = pc_isinvenspace_unequip(sd, fl->id, fl->owner_id, fl->protectnum, fl->real_name);
	} while (fl->amount && num < sd->status.maxinv);
...
int pc_equipitem(USER *sd, int id) {
	...
	if(ret==EQ_WEAP) {
		if(sd->status.equip[EQ_SHIELD].id!=0 && (itemdb_look(sd->status.inventory[id].id)>9999 && itemdb_look(sd->status.inventory[id].id)<30000)) {
			clif_sendminitext(sd, "You cannot wear this weapon with a shield.",0);
			return 0;
		}
	}
	...
	if(ret==EQ_SHIELD){
		if((itemdb_look(sd->status.equip[EQ_WEAP].id)>9999 && itemdb_look(sd->status.equip[EQ_WEAP].id)<30000)) {
			clif_sendminitext(sd, "You cannot wear a shield with this weapon.",0);
			return 0;
		}
	}
...
int pc_getitemscript2(USER *sd, int id) {
	FLOORITEM *fl = map_id2fl(id);
	struct item it;
	int add=0;
	char escape[255];
...
int pc_addtocurrent(struct block_list *bl, va_list ap) {
	...
	if(fl->data.id==sd->status.inventory[id].id && fl->data.owner_id == sd->status.inventory[id].owner_id && fl->data.protectnum == sd->status.inventory[id].protectnum && !(strcmpi(fl->data.real_name, sd->status.inventory[id].real_name))) {
...
int pc_npc_drop(struct block_list* bl, va_list ap) {
	...
	//if(def[1]) return 0;
	if(nd->bl.subtype == 2 || nd->bl.subtype == FLOOR) {
