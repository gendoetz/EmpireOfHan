...
int bll_sendanimxy(lua_State *state, void *self) {
	...
	map_foreachinarea(clif_sendanimation_xy, bl->m, bl->x, bl->y, SAMEMAP, BL_PC, anim, times, x, y);
	return 0;
}
...
int setmap(lua_State *state) {
	...
	int mountallow = lua_tonumber(state, 16);
...
int getMentorCode(lua_State *state) {
	int id;
	char name[16];
	SqlStmt* stmt;
	...
	if (lua_isnumber(state, 1)) {
		...
		char* name=lua_tostring(state, 1);
		if(SQL_ERROR==SqlStmt_Prepare(stmt,"SELECT `ChaId` FROM `Character` WHERE `ChaRefMentor` = '%s'",name)
		|| SQL_ERROR==SqlStmt_Execute(stmt)
		|| SQL_ERROR==SqlStmt_BindColumn(stmt,0,SQLDT_UINT,&id,0,NULL,NULL)) {
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
...
void sl_init() {
	...
	*/
	...
	lua_register(sl_gstate,"getMentorCode",getMentorCode);
...
int sl_async(lua_State *state) {
	...
	if(sd->coref) { sl_async_freeco(sd); };
...
void sl_resumebuy2(char* choice, USER *sd) {
	/*if(sd->coroutine) {
	...
	lua_pushstring(sl_gstate, choice);
	sl_async_resume(sd, 1);
}
...
int iteml_getattr(lua_State *state, struct item_data *item, char *attrname) {
	...
	else if(!strcmp(attrname, "allowDye")) lua_pushnumber(state, item->allowdye);
	else if(!strcmp(attrname, "breakondeath")) lua_pushnumber(state, item->breakondeath);
...
int biteml_setattr(lua_State *state, struct item *bitem, char *attrname) {
	...
	else if(!strcmp(attrname,"protectnum")) bitem->protectnum=lua_tonumber(state,-1);
...
int biteml_getattr(lua_State *state, struct item *bitem, char *attrname) {
	...
	else if(!strcmp(attrname, "protectnum")) lua_pushnumber(state,bitem->protectnum);
...
int parcell_getattr(lua_State *state, struct parcel *parcel, char *attrname) {
	...
	else if(!strcmp(attrname, "protectnum")) lua_pushnumber(state,parcel->data.protectnum);
...
int bll_handle_mob(struct block_list* bl, va_list ap) {
	...
	//printf("Mob data... mob: %u, prot: %u, respawn: %u\n", mob->state, mob->protection, mob->mrespawnfunc);
	//mob->spawncheck=mobdb_spawntime(mob->id);
	if(mob->mrespawnfunc==0 && mob->state==MOB_DEAD && (!mob->onetime)) {
		mob_respawn(mob);
	}
	return 0;
}
int bll_respawn(lua_State* state, struct block_list* bl) {
	...
	map_respawn(bll_handle_mob, m, BL_MOB);
	return 0;
}
...
int bll_sendparcel(lua_State *state, struct block_list *bl) {
	...
	int protectnum = lua_tonumber(state, sl_memberarg(8));
	SqlStmt* stmt = SqlStmt_Malloc(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Parcels` (`ParChaIdDestination`, `ParSender`, `ParItmId`, `ParAmount`, `ParChaIdOwner`, `ParProtectNum`, `ParEngrave`, `ParPosition`, `ParNpc`) VALUES ('%u', '%u', '%u', '%u', '%u', '%u', '%s', '%d', '%d')",
	receiver, sender, item, amount, owner, protectnum, escape, newest, npcflag)) {
...
int bll_getattr(lua_State *state, struct block_list *bl, char *attrname) {
	...
	else if(!strcmp(attrname,"mountallow")) lua_pushnumber(state,map[bl->m].mountallow);
	else return 0; // the attribute was not handled
	return 1;
}
int bll_setattr(lua_State *state, struct block_list *bl, char*attrname) {
	...
	else if(!strcmp(attrname,"mountallow")) map[bl->m].mountallow=lua_tonumber(state,-1);
	else return 0;
	return 1;
}
...
int mobl_setpermspawn(lua_State*, MOB*);
...
void mobl_staticinit() {
	...
	typel_extendproto(&mobl_type,"setPermSpawn",mobl_setpermspawn);
...
int mobl_getattr(lua_State *state, MOB *mob, char *attrname) {
	...
	else if(!strcmp(attrname,"sound")) lua_pushnumber(state,mob->sound);
	...
	else if(!strcmp(attrname,"mrespawnfunc")) lua_pushnumber(state, mob->mrespawnfunc);
...
int mobl_setattr(lua_State *state, MOB *mob, char* attrname) {
	...
	else if(!strcmp(attrname,"mrespawnfunc")) mob->mrespawnfunc = lua_tonumber(state, -1);
	...
	else if(!strcmp(attrname,"sound"))mob->sound=lua_tonumber(state, -1);
...
int mobl_setpermspawn(lua_State* state, MOB* mob) {
	sl_checkargs(state, "nnnn");
	unsigned short m, x, y;
	...
	id = lua_tonumber(state, sl_memberarg(1));
	start = lua_tonumber(state, sl_memberarg(2));
	end = lua_tonumber(state, sl_memberarg(3));
	replace = lua_tonumber(state, sl_memberarg(4));
	//map_addmob(sd, id, start, end, replace);

	if(SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Spawns` (`SpnMapId`, `SpnX`, `SpnY`, `SpnMobId`, `SpnLastDeath`, `SpnStartTime`, `SpnEndTime`, `SpnMobIdReplace`) VALUES(%d, %d, %d, %d, %d, %d, %d, %d)", mob->bl.m, mob->bl.x, mob->bl.y, id, 0, start, end, replace))
		Sql_ShowDebug(sql_handle);
...
int pcl_paper(lua_State*,USER*);
int pcl_paperwrite(lua_State*,USER*);
...
int pcl_checkbankprotects(lua_State*, USER*);
...
int pcl_pickup2(lua_State*, USER*);
...
int pcl_getunknownspells_base(lua_State*, USER*);
int pcl_getunknownspells_sub(lua_State*, USER*);
...
void pcl_staticinit() {
	...
	typel_extendproto(&pcl_type, "paper",pcl_paper);
	typel_extendproto(&pcl_type, "paperWrite",pcl_paperwrite);
	...
	typel_extendproto(&pcl_type, "bBank",pcl_bBank);
	...
	typel_extendproto(&pcl_type, "pickUp2", pcl_pickup2);
	...
	typel_extendproto(&pcl_type, "getUnknownSpells_base", pcl_getunknownspells_base);
	typel_extendproto(&pcl_type, "getUnknownSpells_sub", pcl_getunknownspells_sub);
	...
	typel_extendproto(&pcl_type, "checkBankProtects",pcl_checkbankprotects);
...
int pcl_paper(lua_State *state, USER *sd) {
	sl_checkargs(state,"snn");

	unsigned int width = lua_tonumber(state, sl_memberarg(2));
	unsigned int height = lua_tonumber(state, sl_memberarg(3));

	clif_paperpopupwrite(sd,lua_tostring(state, sl_memberarg(1)), width, height);
	return 0;
}
int pcl_paperwrite(lua_State *state, USER *sd) {
	sl_checkargs(state,"snn");

	unsigned int width = lua_tonumber(state, sl_memberarg(2));
	unsigned int height = lua_tonumber(state, sl_memberarg(3));

	clif_paperpopup(sd,lua_tostring(state, sl_memberarg(1)), width, height);
	return 0;
}
...
int pcl_setattr(lua_State *state, USER *sd, char *attrname) {
	...
	else if(!strcmp(attrname,"tnl")) sd->status.tnl=lua_tonumber(state,-1);
	else if(!strcmp(attrname,"totemTime")) sd->status.totemTime=lua_tonumber(state,-1);
	else if(!strcmp(attrname,"shieldVal")) sd->status.shieldVal=lua_tonumber(state,-1);
	else if(!strcmp(attrname,"email")) strcpy(sd->status.email,lua_tostring(state,-1));
	else if(!strcmp(attrname,"refCode")) strcpy(sd->status.refCode,lua_tostring(state,-1));
	else if(!strcmp(attrname,"helper")) sd->helper=lua_tonumber(state,-1);
...
//	}
	...
	else if(!strcmp(attrname,"tnl")) lua_pushnumber(state,sd->status.tnl);
	else if(!strcmp(attrname,"totemTime")) lua_pushnumber(state,sd->status.totemTime);
	else if(!strcmp(attrname,"shieldVal")) lua_pushnumber(state,sd->status.shieldVal);
	else if(!strcmp(attrname,"email")) lua_pushstring(state,sd->status.email);
	else if(!strcmp(attrname,"refCode")) lua_pushstring(state,sd->status.refCode);
	else if(!strcmp(attrname,"helper")) lua_pushnumber(state,sd->helper);
...
int pcl_bBank(lua_State *state, USER *sd) {
	sl_checkargs(state,"stt");
	int x;
	unsigned int amount=lua_objlen(state,sl_memberarg(2));
	struct item_data *item[amount+1];
	...
	clif_bBankDialog(sd, sd->last_click, dialog, item, var2, amount);
...
int pcl_removeinventoryitem(lua_State *state, USER *sd) {
	unsigned int id = 0, amount = 0, owner = 0, protectnum = 0,max = 0;
	...
	protectnum = lua_tonumber(state, sl_memberarg(6));
	max = amount;
	...
	for (x = 0; x < sd->status.maxinv; x++)
	{
		if (sd->status.inventory[x].id == id && sd->status.inventory[x].amount < itemdb_maxamount(id) && sd->status.inventory[x].owner_id == owner && sd->status.inventory[x].protectnum == protectnum && !(strcmpi(sd->status.inventory[x].real_name, engrave)))
		{
	...
	for (x = 0; x < sd->status.maxinv; x++)
	{
		if (sd->status.inventory[x].id == id && sd->status.inventory[x].owner_id == owner && sd->status.inventory[x].protectnum == protectnum && !(strcmpi(sd->status.inventory[x].real_name, engrave)))
		{
			if (sd->status.inventory[x].amount < amount && sd->status.inventory[x].amount > 0)
			{
				amount -= sd->status.inventory[x].amount;
...
int pcl_hasitem(lua_State *state, USER* sd) {
	unsigned int id = 0, amount = 0, leftover = 0, owner = 0, protectnum =0;
	int x, count = 0;
	char *engrave = NULL;
	...
	protectnum = lua_tonumber(state, sl_memberarg(5));
	leftover = amount;
...
int pcl_hasspace(lua_State* state, USER *sd) {
	...
	engrave = lua_tostring(state,sl_memberarg(4));
	int protectnum=lua_tonumber(state,sl_memberarg(5));
	...
	if(pc_isinvenspace(sd,id, owner, protectnum, engrave)>=sd->status.maxinv) {
		lua_pushboolean(state, 0);
		return 1;
	}
	...
	for (x = 0; x < sd->status.maxinv; x++) {
		...
		if (sd->status.inventory[x].id == id && sd->status.inventory[x].owner_id == owner && sd->status.inventory[x].protectnum == protectnum && engraved == 0) {
...
//check bank owners
int pcl_checkbankprotects(lua_State* state, USER *sd) {
	int i;
	lua_newtable(state);

	for (i = 0; i < MAX_BANK_SLOTS; i++) {
		if (sd->status.banks[i].protectnum > 0) {
			lua_pushnumber(state, sd->status.banks[i].protectnum);
			lua_rawseti(state, -2, i + 1);
		} else {
			lua_pushnumber(state, 0);
			lua_rawseti(state, -2, i + 1);
...
int pcl_bankdeposit(lua_State* state, USER *sd) {
	sl_checkargs(state, "nn");
	int x, deposit, amount, owner, protectnum;
	unsigned int item;
	char *engrave = NULL;
	...
	for (x = 0; x < MAX_BANK_SLOTS;x++) {
		if (sd->status.banks[x].item_id == item && sd->status.banks[x].owner_id == owner && sd->status.banks[x].protectnum == protectnum && !strcmpi(sd->status.banks[x].real_name, engrave)) {
			deposit = x;
			break;
		}
	}
	...
	if (deposit != -1) {
		...
		for (x = 0; x < MAX_BANK_SLOTS; x++) {
			if (sd->status.banks[x].item_id == 0) {
				...
				sd->status.banks[x].protectnum = protectnum;
	...
	if (deposit != -1) {
		if ((sd->status.banks[deposit].amount - amount) < 0) {
			...
			if (sd->status.banks[deposit].amount <= 0) {
				...
				sd->status.banks[deposit].protectnum = 0;
				...
				for (x = 0; x < MAX_BANK_SLOTS; x++) {
					if (sd->status.banks[x].item_id == 0 && x < MAX_BANK_SLOTS - 1) {
						...
						sd->status.banks[x].protectnum = sd->status.banks[x + 1].protectnum;
						sd->status.banks[x + 1].protectnum = 0;
...
int pcl_checkbankitemamount(lua_State* state, USER *sd) {
	...
	protectnum = lua_tonumber(state, sl_memberarg(4));
...
int pcl_pickup2(lua_State *state, USER *sd) {
	sl_checkargs(state, "n");
	unsigned int id = lua_tonumber(state, sl_memberarg(1));
	
	pc_getitemscript2(sd, id);
	return 0;
}
...
int pcl_getparcel(lua_State *state, USER *sd) {
	struct parcel *item;
	unsigned int id, amount, owner_id, sender, protectnum;
	int pos;
	char real_name[64];
	char npcflag;
	SqlStmt* stmt = SqlStmt_Malloc(sql_handle);
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `ParItmId`, `ParAmount`, `ParChaIdOwner`, `ParProtectNum`, `ParEngrave`, `ParSender`, `ParPosition`, `ParNpc` FROM `Parcels` WHERE `ParChaIdDestination` = '%u'", sd->status.id)
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 3, SQLDT_UINT, &protectnum, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_STRING, &real_name, sizeof(real_name), NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UINT, &sender, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 6, SQLDT_INT, &pos, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 7, SQLDT_CHAR, &npcflag, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		lua_pushboolean(state, 0);
		return 1;
	}
	...
	if (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
		...
		item->data.protectnum = protectnum;
...
int pcl_expireitem(lua_State* state, USER* sd) {
	...
	for (x = 0; x < sd->status.maxinv; x++) {
		if (sd->status.inventory[x].timeleft > 0 && sd->status.inventory[x].timeleft < t && itemdb_time(sd->status.inventory[x].id)) {
			sprintf(msg, "Your %s has expired!", itemdb_name(sd->status.inventory[x].id));
			pc_delitem(sd, x, 1, 8);
	...
	for (x = 0; x < 14; x++) {
		if (sd->status.equip[x].timeleft > 0 && sd->status.equip[x].timeleft < t && itemdb_time(sd->status.equip[x].id)) {
			sprintf(msg, "Your %s has expired!", itemdb_name(sd->status.equip[x].id));
			pc_unequip(sd, x);
...
int pcl_getunknownspells_base(lua_State* state, USER* sd) {
	int i, j, x;
	unsigned int id;
	unsigned int idlist[255];
	char found = 0;
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `SplId` FROM `Spells` WHERE `SplPthId` IN ('-1', '0', '%d') AND `SplActive` = '1'", classdb_path(sd->status.class))
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int pcl_getunknownspells_sub(lua_State* state, USER* sd) {
	int i, j, x;
	unsigned int id;
	unsigned int idlist[255];
	char found = 0;
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `SplId` FROM `Spells` WHERE `SplPthId` IN ('-1', '%d') AND `SplActive` = '1'", sd->status.class)
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
