	...
	case BL_ITEM:
		...
		if(item->data.id == 0) {
			if(item->data.amount == 1) {
				WFIFOW(sd->fd,len+16)=SWAP16(itemdb_icon(341));
				WFIFOB(sd->fd,len+18)=itemdb_iconcolor(341);
			}
			if(item->data.amount > 1 && item->data.amount < 100) {
				WFIFOW(sd->fd,len+16)=SWAP16(itemdb_icon(342));
				WFIFOB(sd->fd,len+18)=itemdb_iconcolor(342);
			}
			if(item->data.amount >= 100 && item->data.amount < 1000) {
				WFIFOW(sd->fd,len+16)=SWAP16(itemdb_icon(343));
				WFIFOB(sd->fd,len+18)=itemdb_iconcolor(343);
			}
			if(item->data.amount >= 1000) {
				WFIFOW(sd->fd,len+16)=SWAP16(itemdb_icon(item->data.id));
				WFIFOB(sd->fd,len+18)=itemdb_iconcolor(item->data.id);
			}
		} else {
...
int clif_object_look_sub2(struct block_list *bl, va_list ap) {
	...
	case BL_ITEM:
		...
		if(item->data.id == 0) {
			if(item->data.amount == 1) {
				WFIFOW(sd->fd,16)=SWAP16(itemdb_icon(341));
				WFIFOB(sd->fd,18)=itemdb_iconcolor(341);
			}
			if(item->data.amount > 1 && item->data.amount < 100) {
				WFIFOW(sd->fd,16)=SWAP16(itemdb_icon(342));
				WFIFOB(sd->fd,18)=itemdb_iconcolor(342);
			}
			if(item->data.amount >= 100 && item->data.amount < 1000) {
				WFIFOW(sd->fd,16)=SWAP16(itemdb_icon(343));
				WFIFOB(sd->fd,18)=itemdb_iconcolor(343);
			}
			if(item->data.amount >= 1000) {
...
int clif_send_mob_healthscript(MOB* mob, int damage, int critical) {
	...
	if (!mob->current_vita) {
		...
		if(sd != NULL && mob->summon == 0) {
			...
			//Altering percent exp lowered for higher stat players
			if (sd->status.shieldVal > 0) {

				if (sd->status.shieldVal == 1) {
					calcexp = mob->data->exp * .90;
				}
				if (sd->status.shieldVal == 2) {
					calcexp = mob->data->exp * .80;
				}
				if (sd->status.shieldVal == 3) {
					calcexp = mob->data->exp * .70;
				}
				if (sd->status.shieldVal == 4) {
					calcexp = mob->data->exp * .60;
				}
				if (sd->status.shieldVal == 5) {
					calcexp = mob->data->exp * .50;
				}
				if (sd->status.shieldVal > 5) {
					calcexp = mob->data->exp;
				}
			...
			if(sd->group_count == 0) {
				pc_givexp(sd, calcexp, xp_rate);
				if(calcexp) addtomq(sd, mob->mobid);
			} else if (sd->group_count > 0) {
				clif_groupexp(sd, calcexp);
...
int clif_mystaytus(USER *sd) {
	...
	int maxtnl,path,tnl = sd->status.tnl;
	int helper = sd->helper;
	char *name = NULL;
	char* owner = NULL;
	int tempac=0;
...
int clif_lookgoneWarp(struct block_list *bl) {
	char buf[16];
...
int clif_charlook_sub(struct block_list *bl, va_list ap) {
    ...
    if (!pc_isequip(sd, EQ_ARMOR)) {
		...
		if(itemdb_allowdye(pc_isequip(sd,EQ_ARMOR)) == 1) {
			if(sd->status.armor_color>0) {

				if(itemdb_lookcolor(pc_isequip(sd,EQ_ARMOR))<0) {
					WFIFOB(src_sd->fd,28)=itemdb_lookcolor(pc_isequip(sd,EQ_ARMOR))+sd->status.armor_color;
				} else {
    ...
    if(pc_isequip(sd,EQ_COAT)) {
		...
		if(itemdb_allowdye(pc_isequip(sd,EQ_COAT))	== 1) {
			if(sd->status.armor_color>0) {

				if(itemdb_lookcolor(pc_isequip(sd,EQ_COAT))<0) {
					WFIFOB(src_sd->fd,28)=itemdb_lookcolor(pc_isequip(sd,EQ_COAT))+sd->status.armor_color;
				} else {
					WFIFOB(src_sd->fd,28)=sd->status.armor_color;
				}
...
//shield
	...
	//beard stuff
		...
		if(itemdb_allowdye(pc_isequip(sd,EQ_FACEACC)) == 1) {
			if(sd->status.hair_color>0) {
				if(itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC))<0) {
					WFIFOB(src_sd->fd, 40)=itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC))+sd->status.hair_color; //Crown color
				} else {
					WFIFOB(src_sd->fd, 40)=sd->status.hair_color; //Crown color
				}
			} else {
				WFIFOB(src_sd->fd, 40)=itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC)); //beard color
			}
	...
	if (!pc_isequip(sd,EQ_CROWN)) {
		...
		// old: WFIFOB(src_sd->fd, 43)=itemdb_lookcolor(pc_isequip(sd,EQ_CROWN)); //Crown color
		if(itemdb_allowdye(pc_isequip(sd,EQ_CROWN)) == 1) {
			if(sd->status.hair_color>0) {
				if(itemdb_lookcolor(pc_isequip(sd,EQ_CROWN))<0) {
					WFIFOB(src_sd->fd, 43)=itemdb_lookcolor(pc_isequip(sd,EQ_CROWN))+sd->status.hair_color; //Crown color
				} else {
					WFIFOB(src_sd->fd, 43)=sd->status.hair_color; //Crown color
				}
			} else {
...
int clif_sendstatus(USER* sd, int flags) {
	...
	//	percentage=(((float)(helper-tnl))/(helper))*100;
		percentage=(((float)(sd->helper-sd->status.tnl))/(sd->helper))*100;
		//printf("Helper: %d\n",helper);
		//printf("Tnl: %d\n",tnl);
	} else {
...
int clif_checkdura(USER *sd, int equip) {
	...
	if (percentage <= .5 && percentage > .35 && sd->status.equip[equip].repair == 0) {
		if(sd->status.equip[equip].dura > 0) {
			sprintf(buf, "Your %s is at 50%%.", itemdb_name(id));
			clif_sendminitext(sd, buf, 0);
			clif_sendbluemessage(sd, buf);
		}
		
		sd->status.equip[equip].repair = 1;
	} else if (percentage <= .35 && percentage > .15 && sd->status.equip[equip].repair == 1) {
		if(sd->status.equip[equip].dura > 0) {
	...
	} else if (percentage <= .15 && percentage > .1 && sd->status.equip[equip].repair == 2) {
		if(sd->status.equip[equip].dura > 0) {
	...
	} else if (percentage <= .1 && percentage > .05 && sd->status.equip[equip].repair == 3) {
		if(sd->status.equip[equip].dura > 0) {
	...
	} else if (percentage <= .05 && sd->status.equip[equip].repair == 4) {
		if(sd->status.equip[equip].dura > 0) {
	...
	if(sd->status.equip[equip].dura<=0) {
		...
		sd->status.equip[equip].protectnum=0;
...
*/	
	...
	sd->status.inventory[num].protectnum=0;
...
int clif_sendadditem(USER *sd, int num) {
	...
	if((itemdb_type(id)<3) || (itemdb_type(id)>17)) {
		...
		WFIFOB(sd->fd, len + 14) = sd->status.inventory[num].protectnum; //REPLACE WITH PROTECTED
		len+=6;
	}
...
int clif_sendscriptsay(USER *sd, char *msg, int msglen, int type) {
	...
	if (type >= 10) {
		...
		if (type==1) {
			...
			clif_send(buf, 16+namelen+msglen, &sd->bl, AREA);
		}
	}
...
int clif_parserefresh_ctrlr(USER *sd) {
	sl_doscript_blargs("walkPause", NULL, 2, &sd->bl, 0);
	clif_sendmapinfo(sd);
	clif_sendxy(sd);
	clif_mob_look_start(sd);
	map_foreachinarea(clif_object_look_sub,sd->bl.m,sd->bl.x,sd->bl.y,SAMEAREA,BL_ALL,LOOK_GET,sd);
...
int clif_scriptmenu(USER *sd, unsigned int id, char* dialog, char* menu[], int size) {
...
int clif_scriptmenuseq(USER *sd, unsigned int id, char* dialog, char* menu[], int size, int previous, int next) {
	int graphic_id=sd->npc_g;
	int color=sd->npc_gc;
	int x;
	int len=0;
...
int clif_updatestate(struct block_list *bl, va_list ap) {
		...
		if (!pc_isequip(sd, EQ_ARMOR)) {
			...
			if(itemdb_allowdye(pc_isequip(sd,EQ_ARMOR)) == 1) {
				if(sd->status.armor_color>0) {
					...
					if(itemdb_lookcolor(pc_isequip(sd,EQ_ARMOR))<0) {
						WFIFOB(src_sd->fd,23)=itemdb_lookcolor(pc_isequip(sd,EQ_ARMOR))+sd->status.armor_color;
					} else {
		...
		if(pc_isequip(sd,EQ_COAT)) {
			...
			if(itemdb_allowdye(pc_isequip(sd,EQ_COAT)) == 1) {
				if(sd->status.armor_color>0) {
					...
					if(itemdb_lookcolor(pc_isequip(sd,EQ_COAT))<0) {
						WFIFOB(src_sd->fd,23)=itemdb_lookcolor(pc_isequip(sd,EQ_COAT))+sd->status.armor_color;
					} else {
						WFIFOB(src_sd->fd,23)=sd->status.armor_color;
					}
	...
	//beard stuff
			...
			if(itemdb_allowdye(pc_isequip(sd,EQ_FACEACC)) == 1) {
				if(sd->status.hair_color>0) {
					if(itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC))<0) {
						WFIFOB(src_sd->fd, 35)=itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC))+sd->status.hair_color; //Crown color
					} else {
						WFIFOB(src_sd->fd, 35)=sd->status.hair_color; //Crown color
					}
				} else {
					WFIFOB(src_sd->fd, 35)=itemdb_lookcolor(pc_isequip(sd,EQ_FACEACC)); //beard color
				}
	...
	//crown
		if (!pc_isequip(sd,EQ_CROWN)) {
			...
			if(itemdb_allowdye(pc_isequip(sd,EQ_CROWN)) == 1) {
				if(sd->status.hair_color>0) {
					if(itemdb_lookcolor(pc_isequip(sd,EQ_CROWN))<0) {
						WFIFOB(src_sd->fd, 38)=itemdb_lookcolor(pc_isequip(sd,EQ_CROWN))+sd->status.hair_color; //Crown color
					} else {
						WFIFOB(src_sd->fd, 38)=sd->status.hair_color; //Crown color
					}
				} else {
					WFIFOB(src_sd->fd, 38)=itemdb_lookcolor(pc_isequip(sd,EQ_CROWN)); //Crown color
				}
...
int clif_buydialog(USER* sd,unsigned int id, char* dialog,struct item_data *item[], int var2[], int count) {
	...
	if (graphic > 0) {
		...
		for(x=0;x<count;x++) {
			...
			if(strcmpi(itemdb_buytext(item[x]->id),"") == 0) {
				WFIFOB(sd->fd,len + 22) = strlen(" ");
				strcpy(WFIFOP(sd->fd,len + 23), " ");
				len += strlen(" ") + 1;
			} else {
				WFIFOB(sd->fd,len + 22) = strlen(itemdb_buytext(item[x]->id));
				strcpy(WFIFOP(sd->fd,len + 23), itemdb_buytext(item[x]->id));
...
int clif_bBankDialog(USER* sd,unsigned int id, char* dialog,struct item_data *item[], int var2[], int count) {
	NPC* nd = NULL;
	int graphic=sd->npc_g;
    int color=sd->npc_gc;
   	int x;
	...
	if (graphic > 0) {
		...
		for(x=0;x<count;x++) {
			...
			WFIFOL(sd->fd,len+22)=SWAP32(var2[x]);
			len+=4;
			if(strcmpi(sd->status.banks[x].real_name,"") == 0) {
				if (sd->status.banks[x].owner_id > 0) {
					sprintf(copycat, "%s--Bonded", itemdb_name(item[x]->id));
					WFIFOB(sd->fd,len+22)=strlen(copycat);
					strcpy(WFIFOP(sd->fd,len+23),copycat);
					len+=strlen(copycat)+1;
				} else {
					if (sd->status.banks[x].protectnum == 0) {
						WFIFOB(sd->fd,len+22)=strlen(itemdb_name(item[x]->id));
						strcpy(WFIFOP(sd->fd,len+23),itemdb_name(item[x]->id));
						len+=strlen(itemdb_name(item[x]->id))+1;
					} else {
						sprintf(copycat, "%s--Protected %d", itemdb_name(item[x]->id), sd->status.banks[x].protectnum);
						WFIFOB(sd->fd,len+22)=strlen(copycat);
						strcpy(WFIFOP(sd->fd,len+23),copycat);
						len+=strlen(copycat)+1;
					}
				...
				// = itemdb_name(item[x]->id) + sd->status.banks[x].real_name;
				if (sd->status.banks[x].owner_id > 0) {
					sprintf(copycat, "%s--%s--Bonded", itemdb_name(item[x]->id), sd->status.banks[x].real_name);
				} else {
					sprintf(copycat, "%s--%s", itemdb_name(item[x]->id), sd->status.banks[x].real_name);
				}
...
int clif_parsebuy(USER* sd) {
	...
	//clif_debug(RFIFOP(sd->fd,13),SWAP16(RFIFOW(sd->fd,1)));
...
int clif_clickonplayer(USER* sd,struct block_list *bl) {
    ...
    if (!pc_isequip(tsd, EQ_ARMOR)) {
		...
		if(itemdb_allowdye(pc_isequip(tsd,EQ_ARMOR))	== 1) {
			if(tsd->status.armor_color>0) {

				if(itemdb_lookcolor(pc_isequip(tsd,EQ_ARMOR))<0) {
					WFIFOB(sd->fd,len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_ARMOR))+tsd->status.armor_color;
				} else {
	...
	//New coat check for dye
		if(itemdb_allowdye(pc_isequip(tsd,EQ_COAT))	== 1) {
			if(tsd->status.armor_color>0) {
				if(itemdb_lookcolor(pc_isequip(tsd,EQ_COAT))<0) {
					WFIFOB(sd->fd,len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_COAT))+tsd->status.armor_color;
				} else {
					WFIFOB(sd->fd,len+8)=tsd->status.armor_color;
				}
			} else {
	...
	//beard stuff
		...
		if(itemdb_allowdye(pc_isequip(tsd,EQ_FACEACC)) == 1) {
			if(tsd->status.hair_color>0) {
				if(itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC))<0) {
					WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC))+tsd->status.hair_color; //Crown color
				} else {
					WFIFOB(sd->fd, len+8)=tsd->status.hair_color;
				}
			} else {
				WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC)); //beard color
			}
	...
	if (!pc_isequip(tsd,EQ_CROWN)) {
		...
		if(itemdb_allowdye(pc_isequip(tsd,EQ_CROWN)) == 1) {
			if(tsd->status.hair_color>0) {
				if(itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN))<0) {
					WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN))+tsd->status.hair_color; //Crown color
				} else {
					WFIFOB(sd->fd, len+8)=tsd->status.hair_color;
				}
			} else {
	...
	for (x = 0, n = 0, w = 0, r = 0, m = 0, p = 0, a = 0, g = 0; (n + w + r + m + p + a + g) < sd->group_count; x++) {
		...
		//beard stuff
			...
			if(itemdb_allowdye(pc_isequip(tsd,EQ_FACEACC)) == 1) {
				if(tsd->status.hair_color>0) {
					if(itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC))<0) {
						WFIFOB(sd->fd, len+13)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC))+tsd->status.hair_color; //Crown color
					} else {
						WFIFOB(sd->fd, len+13)=tsd->status.hair_color;
					}
				} else {
					WFIFOB(sd->fd, len+13)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC)); //beard color
				}
		...
		if (!pc_isequip(tsd,EQ_CROWN)) {
			...
			if(itemdb_allowdye(pc_isequip(tsd,EQ_CROWN)) == 1) {
				if(tsd->status.hair_color>0) {
					if(itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN))<0) {
						WFIFOB(sd->fd, len+16)=itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN))+tsd->status.hair_color; //Crown color
					} else {
						WFIFOB(sd->fd, len+16)=tsd->status.hair_color;
					}
				} else {
					WFIFOB(sd->fd, len+16)=itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN)); //Crown color
				}
...
int clif_groupexp(USER *sd, unsigned int xp) {
	...
	int highest=0;
// roy added this
	int group_onmap = sd->group_count;


	float percentage;
	
	if(!xp) return 0;
//roy added code to only give exp if on the same map
...
//}
	...
	group_xp = (xp + (xp * .01 * group_onmap)) * pow(.9 - (.01 * group_onmap / 2), group_onmap);
