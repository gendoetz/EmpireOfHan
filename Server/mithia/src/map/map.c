...
struct map_src_list{
	...
	char title[64];
	unsigned char cantalk, show_ghosts, region, indoor, warpout, bind, mountallow;
...
int map_src_add(const char *r1) {
	...
	unsigned char cantalk,showghosts,region,indoor,warpout, bind, mountallow;
	struct map_src_list *new;
	char map_title[1024], map_file[1024];
	if (sscanf(r1, "%d,%[^,],%d,%d,%d,%u,%u,%u,%u,%u,%u,%u,%u, %u,%s", &map_id, map_title, &map_bgm, &pvp, &spell, &light, &weather, &sweeptime, &cantalk, &showghosts, &region, &indoor, &warpout, &bind, &mountallow, map_file) < 13) {
		return -1;
	}
//[^,],[^\r\n]
...
int map_read() {//int id, const char *title, char bgm, int pvp, int spell, unsigned short light, unsigned short weather, unsigned int sweeptime, unsigned char cantalk, unsigned char showghosts, unsigned char region, unsigned char indoor, unsigned char warpout, const char *map_file) {
	...
	unsigned short bgm;
	unsigned char pvp, spell, light, weather, cantalk, show_ghosts, region, indoor, warpout, bind, mountallow;
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MapId`, `MapName`, `MapBGM`, `MapPvP`, `MapSpells`, `MapLight`, `MapWeather`, `MapSweepTime`, `MapChat`, `MapGhosts`, `MapRegion`, `MapIndoor`, `MapWarpout`, `MapBind`, `MapMountAllow`, `MapFile` FROM `Maps`")
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 14, SQLDT_UCHAR, &mountallow, 0, NULL, NULL)
...
int map_reload() {
	...
	for (i = 0; i < map_n && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
		...
		if (map[id].mountallow != mountallow) map[id].mountallow = mountallow;
...
int map_reset_timer(int v1,int v2) {
	...
	diff+=v2;
	if(reset<=250) {
		clif_broadcast("((Please be patient while the server reboots.))", -1);
	}
	if(reset<=0) {
		for(x=0;x<fd_max;x++) {
		if(session[x] && (sd=session[x]->session_data) && !session[x]->eof) {
