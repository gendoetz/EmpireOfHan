...
int mmo_char_fromdb(unsigned int id, struct mmo_charstatus *p, char *login_name) {
	...
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `ChaName`, `ChaClnId`, `ChaClanTitle`, `ChaTitle`, `ChaF1Name`, `ChaLevel`, `ChaPthId`, `ChaMark`, " //0-7
			...
			"`ChaExperienceSold`, `Chabasemight`, `Chabasegrace`, `Chabasewill`, `Chatnl`, `ChatotemTime`, `ChashieldVal` FROM `Character` WHERE `ChaId` = '%u' LIMIT 1", id) //47
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 48, SQLDT_UINT, &a.basemight, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 49, SQLDT_UINT, &a.basegrace, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 50, SQLDT_UINT, &a.basewill, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 51, SQLDT_UINT, &a.tnl, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 52, SQLDT_UINT, &a.totemTime, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 53, SQLDT_UINT, &a.shieldVal, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 54, SQLDT_STRING, 	&a.email, 		sizeof(a.email),			NULL,	NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 55, SQLDT_STRING, 	&a.refCode, 	sizeof(a.refCode),			NULL,	NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		p->id = 0;
		return -1;
	...
	//Read Bank
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `BnkEngrave`,`BnkItmId`,`BnkAmount`,`BnkChaIdOwner`, `BnkProtectNum`, `BnkPosition` FROM `Banks` WHERE `BnkChaId` = '%u' LIMIT 255", id)
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_UINT, &bank.protectnum, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UCHAR, &pos, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		p->id = 0;
		return -1;
	...
	//Read Inventory
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `InvEngrave`,`InvItmId`, `InvAmount`, `InvDurability`, `InvChaIdOwner`, `InvChaProtectNum`, `InvTimer`, `InvPosition` FROM `Inventory` WHERE `InvChaId` = '%u' LIMIT 52", id)
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UINT, &item.protectnum, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 6, SQLDT_UINT, &item.custom, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 7, SQLDT_UCHAR, &pos, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		p->id = 0;
		return -1;
	...
	//Equip Read
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `EqpEngrave`,`EqpItmId`, '1', `EqpDurability`, `EqpChaIdOwner`, `EqpChaProtectNum`, `EqpTimer`, `EqpSlot` FROM `Equipment` WHERE `EqpChaId` = '%u' LIMIT 14", id)
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &item.real_name, sizeof(item.real_name), NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_UINT, &item.id, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 2, SQLDT_UINT, &item.amount, 0, NULL, NULL)
	...
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_UINT, &item.owner_id, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UINT, &item.protectnum, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 6, SQLDT_UINT, &item.timeleft, 0, NULL, NULL)
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, 7, SQLDT_UCHAR, &pos, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		p->id=0;
	...
	for (i = 0; i < max; i++) {
		...
		if (save_id[i] == i) {
			if(bank[i].item_id == 0) {
				...
				if(SQL_ERROR == Sql_Query(sql_handle,"UPDATE `Banks` SET `BnkItmId` = '%u', `BnkAmount` = '%u', `BnkChaIdOwner` = '%u', `BnkProtectNum` = '%u', `BnkEngrave` = '%s' WHERE `BnkChaId` = '%u' AND `BnkPosition` = '%d'",
					bank[i].item_id, bank[i].amount, bank[i].owner_id, bank[i].protectnum, escape, id, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
			...
			if(bank[i].item_id > 0) {
				if(SQL_ERROR == Sql_Query(sql_handle,"INSERT INTO `Banks` (`BnkChaId`, `BnkItmId`, `BnkAmount`, `BnkChaIdOwner`, `BnkProtectNum`, `BnkEngrave`, `BnkPosition`) VALUES ('%u', '%u', '%u', '%u', '%u', '%s', '%d')",
					id, bank[i].item_id, bank[i].amount, bank[i].owner_id, bank[i].protectnum, escape, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
	...
	for (i = 0; i < max; i++) {
		...
		if (save_id[i] == i) {
			if(items[i].id == 0) {
				...
				if(SQL_ERROR == Sql_Query(sql_handle,"UPDATE `Inventory` SET `InvItmId` = '%u', `InvAmount` = '%u', `InvDurability` = '%u', `InvChaIdOwner` = '%u', `InvEngrave` = '%s', `InvChaProtectNum` = '%u' WHERE `InvChaId` = '%u' AND `InvPosition` = '%d'",
					items[i].id, items[i].amount, items[i].dura, items[i].owner_id, items[i].protectnum,  escape, id, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
		...
		{
			if(items[i].id > 0) {
				if(SQL_ERROR == Sql_Query(sql_handle,"INSERT INTO `Inventory` (`InvChaId`, `InvItmId`, `InvAmount`, `InvDurability`, `InvChaIdOwner`, `InvEngrave`, `InvChaProtectNum`, `InvPosition`) VALUES ('%u', '%u', '%u', '%u', '%u', '%s', '%u', '%u')",
					id, items[i].id, items[i].amount, items[i].dura, items[i].owner_id, items[i].protectnum, escape, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
	...
	for (i = 0; i < max; i++) {
		...
		if (save_id[i] == i) {
			if(items[i].id == 0) {
				...
				if(SQL_ERROR == Sql_Query(sql_handle,"UPDATE `Equipment` SET `EqpItmId` = '%u', `EqpDurability` = '%u', `EqpChaIdOwner` = '%u', `EqpChaProtectNum` = '%u', `EqpEngrave` = '%s' WHERE `EqpChaId` = '%u' AND `EqpSlot` = '%d'",
					items[i].id, items[i].dura, items[i].owner_id, items[i].protectnum,  escape, id, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
		...
		{
			if(items[i].id > 0) {
				if(SQL_ERROR == Sql_Query(sql_handle,"INSERT INTO `Equipment` (`EqpChaId`, `EqpItmId`, `EqpDurability`, `EqpChaIdOwner`, `EqpChaProtectNum`, `EqpEngrave`, `EqpSlot`) VALUES ('%u', '%u', '%u', '%u', '%u', '%s', '%u')",
					id, items[i].id, items[i].dura, items[i].owner_id, items[i].protectnum, escape, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
...
int mmo_char_todb(struct mmo_charstatus *p) {
	...
	Sql_EscapeString(sql_handle,escape5,p->email);
	Sql_EscapeString(sql_handle,escape6,p->refCode);
	...
	if(SQL_ERROR == Sql_Query(sql_handle,"UPDATE `Character` SET `ChaName` = '%s', `ChaClnId` = '%u', `ChaClanTitle` = '%s', `ChaTitle` = '%s', `ChaLevel` = '%u', `ChaPthId` = '%u', `ChaMark` = '%u', "
		...
		"`ChaKillsPK` = '%u', `ChaPKDuration` = '%u', `ChaMuted` = '%u', `ChaHeroes` = '%u', `ChaTier` = '%u', `ChaExperienceSold` = '%llu', `Chabasemight` = '%u', `Chabasegrace` = '%u', `Chabasewill` = '%u', `Chatnl` = '%u', `ChatotemTime` = '%u', `ChashieldVal` = '%u', `Chaemail` = '%s', `ChaRefCode` = '%s' WHERE `ChaId` = '%u'",
			...
			p->killspk, p->pkduration, p->mute, p->heroes, p->tier, p->expsold, p->basemight, p->basegrace, p->basewill, p->tnl, p->totemTime, p->shieldVal, escape5, escape6, p->id))
	{
		Sql_ShowDebug(sql_handle);
		return 0;
	}
