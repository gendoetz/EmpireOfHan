...
struct item {
	unsigned int id, owner_id, custom, timeleft, protectnum;
	int dura, amount;
	char repair;
	char real_name[64];
};
...
//for sql banks
struct bank_data {
	unsigned int item_id, amount, owner_id, timeleft, protectnum;
	char real_name[64];
};
...
struct mmo_charstatus {
	unsigned int id, spouse, clan_id, hp, basehp, mp, basemp, exp, money, maxslots, bankmoney, basemight, basewill, basegrace, tnl, totemTime, shieldVal;
	unsigned int killedby, killspk, pkduration;
	char name[16], f1name[16], title[32], clan_title[32], gm_level, sex, country, state, side, karma, clan_chat, email[255], refCode[32];
