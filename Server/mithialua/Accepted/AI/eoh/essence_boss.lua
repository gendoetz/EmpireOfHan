...
after_death = function(mob, attacker, block)
	mob:sendAnimationXY(148,mob.x,mob.y,1)
end,
...
move = function(mob, target)
	...
	if (target.m == mob.m) then
		moved = essence_boss1.zap(mob, target)
	end
...
zap = function(mob, target)
	...
	--mob:talk(2,"zaptime: "..mob.registry["zapCastTime"].." wither aethers "..mob.registry["zapCastTime"] + aethers.." os time: "..os.time())
		...
		if (#mobBlocks > 0) then
			...
			mob:talk(2,"Desperate Essence: Must.. have.. spirit.. essence!")
			end
		if (rangeCheck and target ~= nil and distance(mob, target) <= range
			...
			if (target.magic > damage) then
				target.magic = target.magic - damage
				target:sendStatus()
			end
...
move = function(mob, target)
	...
	if (target.m == mob.m) then
		moved = essence_boss2.zap(mob, target)
	end
...
attack = function(mob, target)
	mob.state = MOB_ALIVE
	essence_boss2.move(mob, target)
end,

zap = function(mob, target)
	local mobBlocks = mob:getObjectsInArea(BL_PC)
	local rangeCheck = false
	local damage = math.random(200, 310)
	local aethers = math.random(2, 3)
	local range = 5
	...
	--mob:talk(2,"zaptime: "..mob.registry["zapCastTime"].." wither aethers "..mob.registry["zapCastTime"] + aethers.." os time: "..os.time())
		...
		if (#mobBlocks > 0) then
			...
			if(math.random(0,6) == 0) then
			mob:talk(2,"Angry Essence: Must.. have.. life.. essence!")
			end
		if (rangeCheck and target ~= nil and distance(mob, target) <= range
			and os.time() >= mob.registry["zapCastTime"] + aethers) then
			--mob:sendAction(6, 40)
