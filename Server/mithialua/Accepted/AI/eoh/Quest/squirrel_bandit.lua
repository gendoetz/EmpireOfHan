		...
		if (#itemsFound > 0) then
			for i = 1, #itemsFound do
				...
				--	end
					...
					toXYdirect(mob, target.x, target.y)
				end
			end
		else
			mob.returning = false
			...
			local itemcell = mob:getObjectsInCell(mob.m, mob.x, mob.y, BL_ITEM)
				...
				if (itemcell[2] ~= nil) then
					...
					if (bomb == 1) then
						...
						mob:addNPC("ded_squirrel", mob.m, mob.x, mob.y, 500, 4000)
						...
						local random_s = math.random(1,2)
						if (random_s == 1) then
							npc2:talk(1,"Squirrel Bandit: Squeeeeeeeee!")
						else
							npc2:talk(1,"Squirrel Bandit: Kukk-queee! -thwunk-")
						end
