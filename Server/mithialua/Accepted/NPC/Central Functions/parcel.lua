function Player.sendParcelTo(player, npc)
	...
	local amount = 1
	if (item.droppable == true) then
		player:dialog("You cannot send this item as a parcel.", {})
	end
	if (item.maxAmount > 1) then
		amount = math.abs(tonumber(math.floor(player:input("How many "..item.name.." would you like to send?"))))
		if (amount <= 0) then
	...
	if (receiver ~= false) then
		if (player:hasItem(item.id, amount, item.owner, item.realName, item.protectnum) ~= true) then
			player:dialog("You only have "..player:hasItem(item.id, amount, item.owner, item.realName, item.protectnum).." "..item.name"(s).", {})
		elseif (player:hasItemDura(item.id, amount) ~= true) then
			...
			if (player:sendParcel(receiver, player.ID, item.id, amount, item.owner, item.realName, 0, item.protectnum) == true) then
				if (amount > item.maxAmount) then
					player:removeItem(item.id, amount, 2, item.owner, item.realName, item.protectnum)
				else
...
function Player.receiveParcelFrom(player, npc)
	...
	if (player:hasSpace(item.id, item.amount, item.owner, item.realName, item.protectnum)) then
		player:addItem(item.id, item.amount, 0, item.owner, item.realName, item.protectnum)
