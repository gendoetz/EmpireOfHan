...
click = async(function(player, npc)
	...
	choice = player:menuString("Would you like to participate in tower battle? The fee is 1000 coins.", options)
	if (choice == "Yes") then
		if (player:removeGold(1000) == true) then
			...
			local participations = player.registry["towerbattleParticipations"] + 1
			...
			player:removeLegendbyName("towerbattleParticipations")
			
			if (participations == 1) then
				player:addLegend("Participated in "..participations.." Tower Battle.", "towerbattleParticipations", 1, 16)
			else
				player:addLegend("Participated in "..participations.." Tower Battles.", "towerbattleParticipations", 1, 16)
			end
			
			player.registry["arenaDyeSave"] = player.armorColor
			player.armorColor = 0
			player.registry["inCarnage"] = 1
			player.registry["towerbattleParticipations"] = participations
