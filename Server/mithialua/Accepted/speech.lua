...
--normal player
	if(player.gmLevel == 0) then
		if (string.match(speech, "dps (%d+)") ~= nil and string.sub(speech, 0, 3) == "dps") then
	...
	if (player.gmLevel >= 50) then
		--GM commands
		if (lspeech == "/svnload") then
			os.execute("svn checkout http://198.211.104.128/svn/eoh/lua /home/Server/mithialua/Developers/svn --username luacode --password loadlua")
			player.speech = "/svnload"
			player:gmMsg("<Console>: "..player.name.." has used SVN checkout to build SVN files.", 50)
			printf = 0
		elseif (lspeech == "reload") or (lspeech == "/reload") then
	...
	if (player:staff() ~= -1) then
		...
		elseif (speech=="psdye")then
			...
			player:refresh()
			printf = 0
		elseif (speech=="nwdye")then 
			--if (player.gfxClone == 1) then
			--player:talk(0,"what the "..player.gfxClone)
				player.gfxWeapC = player.gfxWeapC + 1
				player:sendMinitext("weap Color #: "..player.gfxWeapC)
			--else
				--player.armorColor = player.armorColor + 1
			--end
	...
	--[[elseif (string.find(speech,"Fuck")) then
		player.speech = string.gsub(speech,"Fuck","Oh poo")
		caps = 1
	elseif (string.find(speech,"FUCK")) then
		player.speech = string.gsub(speech,"FUCK","OH POO")
		caps = 1
	elseif (caps == 0) then
		if (string.find(lspeech,"fuck")) then
			player.speech = string.gsub(lspeech,"fuck","oh poo")
		elseif (string.find(speech,"rofl")) then
			player.speech = string.gsub(speech,"rofl","*Falls onto the floor laughing*")
		end]]
	--end

	--[[if (string.find(string.lower(player.speech),"rofl")) then
		player.speech = string.gsub(string.lower(player.speech),"rofl","*Falls onto the floor laughing*")
	end

	if (string.find(string.lower(player.speech),"fuck")) then
		player.speech = string.gsub(string.lower(player.speech),"fuck","boo")
	end

	if (string.find(string.lower(player.speech),"lmao")) then
		player.speech = string.gsub(string.lower(player.speech),"lmao","*Laughs hysterically*")
	end

	if (string.find(string.lower(player.speech),"cunt")) then
		player.speech = string.gsub(string.lower(player.speech),"cunt","beautiful person")
	end

	if (string.find(string.lower(player.speech),"slut")) then
		player.speech = string.gsub(string.lower(player.speech),"slut","beautiful person")
	end

	if (string.find(string.lower(player.speech),"asshole")) then
		player.speech = string.gsub(string.lower(player.speech),"cunt","cool person")
	end

	if (string.find(string.lower(player.speech),"whore")) then
		player.speech = string.gsub(string.lower(player.speech),"whore","friendly person")
	end

	if (string.find(string.lower(player.speech),"bitch")) then
		player.speech = string.gsub(string.lower(player.speech),"bitch","wonderful person")
	end

	if (string.find(string.lower(player.speech),"faggot")) then
		player.speech = string.gsub(string.lower(player.speech),"faggot","awesome person")
	end

	if (string.find(string.lower(player.speech),"fag")) then
		player.speech = string.gsub(string.lower(player.speech),"fag","awesome person")
	end

	if (string.find(string.lower(player.speech),"lol")) then
		local savespeech = player.speech
		if (string.find(string.lower(player.speech)," lol")) then
			player.speech = string.gsub(string.lower(player.speech),"lol","*Laughs*")
		end
		if (string.find(string.lower(player.speech),"lol ")) then
			player.speech = string.gsub(string.lower(player.speech),"lol","*Laughs*")
		end
		...
		if (string.find(string.lower(savespeech),"(%a)lol(%a)")) then
			player.speech = savespeech
		end
		if (string.find(string.lower(savespeech),"lol(%a)")) then
			player.speech = savespeech
		end
		if (string.find(string.lower(savespeech),"(%a)lol")) then
			player.speech = savespeech
		end
	end

	local length_filter = string.len(player.speech)
	local max_filter = 70
	local total_sub = 0

	total_sub = length_filter - max_filter

	if (total_sub <= 0) then
		total_sub = max_filter
	end

	local filtered_talk = string.sub(player.speech, 0, total_sub)

	player.speech = filtered_talk]]--


	local replacemsg = {"Bai gurl bai!!!",
	"*Stares off into space drooling*",
	"Be a doll and rub my corns for me dear?"}

	local rmess = math.random(#replacemsg)

	if (player:hasDuration("gm_remove_voice")) then
			player:talk(0, ""..player.name..": "..replacemsg[rmess])
			printf = 0
	end
