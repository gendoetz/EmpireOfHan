...
thrown = function(player, target)
	...
	local bowRange = 6 --max range of bow
	local maxTargets = 2 --max targets to acquire and hit, will not exceed bowRange
		...
		for x = 1, #targetList do
			if (targetList[x] ~= nil) then
				...
				if (player.critChance > 0) then
					...
					player.damage = (player.damage / #targetList) * 1.5
