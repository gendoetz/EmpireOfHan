...
swingDamage = function(block, target, printf)
	--local baseDamage = 1 + (block.might * 4) + (block.grace * 3)  --Original Base Damage
	local baseDamage = 1 + (block.might * 2) + block.grace
	local finalDamage
	...
	if (block.blType == BL_PC) then
		...
		local weaponDamage = math.floor(mathRandom(block.minDam / 2, (block.maxDam + block.dam) / 2, 1000) + mathRandom(block.minDam / 2, (block.maxDam + block.dam) / 2, 1000))
		--block:talk(0,"Damage Stat: "..block.dam)
	...
	if (target ~= nil and block.critChance > 0) then
		...
		--local deduction = 1 * string.format("%.2f", target.armor / (target.armor + 400 + 95 * (block.level + block.tier^2 + block.mark^3)))
		local deduction = 1 - ((target.armor * acPerArmor / 100))
		...
		--Static Recoil
		if (target:hasDuration("static_recoil")) then
			if (target.registry["static_recoil"] > 1) then
				if (block:hasDuration("paralysis") == false) then
					if (block.blType == BL_MOB) then
						block:setDuration("paralysis", 20000)
						block:sendAnimation(123, 0)
						block.paralyzed = true
						target.registry["static_recoil"] = target.registry["static_recoil"] - 1

					elseif (block.blType == BL_PC) then
						block:setDuration("paralysis", 4000)
						block:sendAnimation(123, 0)
						block.paralyzed = true
						target.registry["static_recoil"] = target.registry["static_recoil"] - 1
			...
			elseif (target.registry["static_recoil"] == 1) then
				target:setDuration("static_recoil", 0)
			end
		end
	--End Static Recoil

	--Damage at front = less damage
		if (target.side - block.side == 2 or target.side - block.side == -2) then
			finalDamage = finalDamage - (finalDamage * .15)
		end

	--end less damage test
