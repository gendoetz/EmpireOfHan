...
canPush_itemno = function(block, target, push)
	if (target == nil) then
		return nil
	elseif (target.blType == BL_NPC and (target.subType == 2 or target.subType == 3)) then
		return false
...
fourPush_itemno = function(block, blockType)
	local blocksNorth = block:getObjectsInCell(block.m, block.x, block.y - 1, blockType)
	local blocksEast = block:getObjectsInCell(block.m, block.x + 1, block.y, blockType)
	local blocksWest = block:getObjectsInCell(block.m, block.x - 1, block.y, blockType)
	local blocksSouth = block:getObjectsInCell(block.m, block.x, block.y + 1, blockType)
	...
	if (#blocksNorth > 0) then
		if(blocksNorth[1].protection == 1 or blocksNorth[1].mrespawnfunc == 1) then

			player:sendMinitext("This target would break your polearm in half if you tried...")
		else
			block.side = 0
			canPush_itemno(block, blocksNorth[1])
		end
	end

	if (#blocksEast > 0) then
		if(blocksEast[1].protection == 1 or blocksEast[1].mrespawnfunc == 1) then

			player:sendMinitext("This target would break your polearm in half if you tried...")
		else
			block.side = 1
			canPush_itemno(block, blocksEast[1])
		end
	end
	
	if (#blocksSouth > 0) then
		if(blocksSouth[1].protection == 1 or blocksSouth[1].mrespawnfunc == 1) then

			player:sendMinitext("This target would break your polearm in half if you tried...")
		else
			block.side = 2
			canPush_itemno(block, blocksSouth[1])
		end
	end
	
	if (#blocksWest > 0) then
		if(blocksWest[1].protection == 1 or blocksWest[1].mrespawnfunc == 1) then
			player:sendMinitext("This target would break your polearm in half if you tried...")
		else
			block.side = 3
			canPush_itemno(block, blocksWest[1])
		end
	end
