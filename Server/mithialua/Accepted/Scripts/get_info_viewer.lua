...
getTargetsAroundDiamond = function(block, blockType)

	--x4 tiles around target
	local northBlocks = block:getObjectsInCell(block.m, block.x, block.y - 1, blockType)
	local eastBlocks = block:getObjectsInCell(block.m, block.x + 1, block.y, blockType)
	local southBlocks = block:getObjectsInCell(block.m, block.x, block.y + 1, blockType)
	local westBlocks = block:getObjectsInCell(block.m, block.x - 1, block.y, blockType)

	--x8tiles around target
	local northBlocks2 = block:getObjectsInCell(block.m, block.x, block.y - 2, blockType)
	local eastBlocks2 = block:getObjectsInCell(block.m, block.x + 2, block.y, blockType)
	local southBlocks2 = block:getObjectsInCell(block.m, block.x, block.y + 2, blockType)
	local westBlocks2 = block:getObjectsInCell(block.m, block.x - 2, block.y, blockType)
	local northBlocksA = block:getObjectsInCell(block.m, block.x + 1, block.y - 1, blockType)
	local eastBlocksA = block:getObjectsInCell(block.m, block.x + 1, block.y + 1, blockType)
	local southBlocksA = block:getObjectsInCell(block.m, block.x - 1, block.y + 1, blockType)
	local westBlocksA = block:getObjectsInCell(block.m, block.x - 1, block.y - 1, blockType)

	--x16 around target
	local northBlocks3 = block:getObjectsInCell(block.m, block.x, block.y - 3, blockType)
	local eastBlocks3 = block:getObjectsInCell(block.m, block.x + 3, block.y, blockType)
	local southBlocks3 = block:getObjectsInCell(block.m, block.x, block.y + 3, blockType)
	local westBlocks3 = block:getObjectsInCell(block.m, block.x - 3, block.y, blockType)

	local northBlocksA2 = block:getObjectsInCell(block.m, block.x + 1, block.y - 2, blockType)
	local northBlocksA3 = block:getObjectsInCell(block.m, block.x + 2, block.y - 1, blockType)

	local eastBlocksA2 = block:getObjectsInCell(block.m, block.x + 1, block.y + 2, blockType)
	local eastBlocksA3 = block:getObjectsInCell(block.m, block.x + 2, block.y + 1, blockType)

	local southBlocksA2 = block:getObjectsInCell(block.m, block.x - 1, block.y + 2, blockType)
	local southBlocksA3 = block:getObjectsInCell(block.m, block.x - 2, block.y + 1, blockType)

	local westBlocksA2 = block:getObjectsInCell(block.m, block.x - 1, block.y - 2, blockType)
	local westBlocksA3 = block:getObjectsInCell(block.m, block.x - 2, block.y - 1, blockType)
	...
	if (#northBlocks2 > 0) then
		for i = 1, #northBlocks2 do
			table.insert(returnTable, northBlocks2[i])
		end
	end
	
	if (#eastBlocks2 > 0) then
		for i = 1, #eastBlocks2 do
			table.insert(returnTable, eastBlocks2[i])
		end
	end
	
	if (#southBlocks2 > 0) then
		for i = 1, #southBlocks2 do
			table.insert(returnTable, southBlocks2[i])
		end
	end
	
	if (#westBlocks2 > 0) then
		for i = 1, #westBlocks2 do
			table.insert(returnTable, westBlocks2[i])
		end
	end
	--Even more

	if (#northBlocksA > 0) then
		for i = 1, #northBlocksA do
			table.insert(returnTable, northBlocksA[i])
		end
	end
	
	if (#eastBlocksA > 0) then
		for i = 1, #eastBlocksA do
			table.insert(returnTable, eastBlocksA[i])
		end
	end
	
	if (#southBlocksA > 0) then
		for i = 1, #southBlocksA do
			table.insert(returnTable, southBlocksA[i])
		end
	end
	
	if (#westBlocksA > 0) then
		for i = 1, #westBlocksA do
			table.insert(returnTable, westBlocksA[i])
		end
	end

	--Adding x16 diamond

	if (#northBlocks3 > 0) then
		for i = 1, #northBlocks2 do
			table.insert(returnTable, northBlocks2[i])
		end
	end
	
	if (#eastBlocks3 > 0) then
		for i = 1, #eastBlocks2 do
			table.insert(returnTable, eastBlocks2[i])
		end
	end
	
	if (#southBlocks3 > 0) then
		for i = 1, #southBlocks2 do
			table.insert(returnTable, southBlocks2[i])
		end
	end
	
	if (#westBlocks3 > 0) then
		for i = 1, #westBlocks2 do
			table.insert(returnTable, westBlocks2[i])
		end
	end
	...
	if (#northBlocksA2 > 0) then
		for i = 1, #northBlocksA do
			table.insert(returnTable, northBlocksA[i])
		end
	end

	if (#northBlocksA3 > 0) then
		for i = 1, #northBlocksA do
			table.insert(returnTable, northBlocksA[i])
		end
	end
	
	if (#eastBlocksA2 > 0) then
		for i = 1, #eastBlocksA do
			table.insert(returnTable, eastBlocksA[i])
		end
	end

	if (#eastBlocksA3 > 0) then
		for i = 1, #eastBlocksA do
			table.insert(returnTable, eastBlocksA[i])
		end
	end
	
	if (#southBlocksA2 > 0) then
		for i = 1, #southBlocksA do
			table.insert(returnTable, southBlocksA[i])
		end
	end

	if (#southBlocksA3 > 0) then
		for i = 1, #southBlocksA do
			table.insert(returnTable, southBlocksA[i])
		end
	end
	
	if (#westBlocksA2 > 0) then
		for i = 1, #westBlocksA do
			table.insert(returnTable, westBlocksA[i])
		end
	end

	if (#westBlocksA3 > 0) then
		for i = 1, #westBlocksA do
			table.insert(returnTable, westBlocksA[i])
		end
	end
...
getTargetsDiamondedge = function(block, blockType)
	local northBlocks2 = block:getObjectsInCell(block.m, block.x, block.y - 2, blockType)
	local eastBlocks2 = block:getObjectsInCell(block.m, block.x + 2, block.y, blockType)
	local southBlocks2 = block:getObjectsInCell(block.m, block.x, block.y + 2, blockType)
	local westBlocks2 = block:getObjectsInCell(block.m, block.x - 2, block.y, blockType)
...
getTargetsAroundPhase = function(block, blockType, phase)

	local returnTable = {}
	--x4 tiles around target
	if (phase == 1 or phase == 10) then
		local northBlocks = block:getObjectsInCell(block.m, block.x, block.y - 1, blockType)
		local eastBlocks = block:getObjectsInCell(block.m, block.x + 1, block.y, blockType)
		local southBlocks = block:getObjectsInCell(block.m, block.x, block.y + 1, blockType)
		local westBlocks = block:getObjectsInCell(block.m, block.x - 1, block.y, blockType)
	...
	if (phase == 3 or phase == 10) then
		...
		local westBlocksA3 = block:getObjectsInCell(block.m, block.x - 2, block.y - 1, blockType)
			...
			if (#northBlocks3 > 0) then
				for i = 1, #northBlocks3 do
					table.insert(returnTable, northBlocks3[i])
				end
			end
			
			if (#eastBlocks3 > 0) then
				for i = 1, #eastBlocks3 do
					table.insert(returnTable, eastBlocks3[i])
				end
			end
			
			if (#southBlocks3 > 0) then
				for i = 1, #southBlocks3 do
					table.insert(returnTable, southBlocks3[i])
				end
			end
			
			if (#westBlocks3 > 0) then
				for i = 1, #westBlocks3 do
					table.insert(returnTable, westBlocks3[i])
				end
			end
			--Even more

			if (#northBlocksA2 > 0) then
				for i = 1, #northBlocksA2 do
					table.insert(returnTable, northBlocksA2[i])
				end
			end

			if (#northBlocksA3 > 0) then
				for i = 1, #northBlocksA3 do
					table.insert(returnTable, northBlocksA3[i])
				end
			end
			
			if (#eastBlocksA2 > 0) then
				for i = 1, #eastBlocksA2 do
					table.insert(returnTable, eastBlocksA2[i])
				end
			end

			if (#eastBlocksA3 > 0) then
				for i = 1, #eastBlocksA3 do
					table.insert(returnTable, eastBlocksA3[i])
				end
			end
			
			if (#southBlocksA2 > 0) then
				for i = 1, #southBlocksA2 do
					table.insert(returnTable, southBlocksA2[i])
				end
			end

			if (#southBlocksA3 > 0) then
				for i = 1, #southBlocksA3 do
					table.insert(returnTable, southBlocksA3[i])
				end
			end
			
			if (#westBlocksA2 > 0) then
				for i = 1, #westBlocksA2 do
					table.insert(returnTable, westBlocksA2[i])
				end
			end

			if (#westBlocksA3 > 0) then
				for i = 1, #westBlocksA3 do
					table.insert(returnTable, westBlocksA3[i])
				end
			end
	end
...
curT=function()
	timemark = "(Asako "..curYear()..", "..getCurSeason()..")"
	return timemark
end
