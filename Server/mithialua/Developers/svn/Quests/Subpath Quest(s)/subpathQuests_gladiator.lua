...
click = function(player, npc)
	...
	local sG_CoatM = {graphic = convertGraphic(4042, "item"), color = 7}
	local sG_CoatF = {graphic = convertGraphic(4043, "item"), color = 7}
	local sG_Weapon = {graphic = convertGraphic(2363, "item"), color = 0}

	local subCoat = player.quest["subitem_Coat"]
	local subWeapon = player.quest["subitem_Weapon"]

	local className = "Gladiator"

	local subCM_Name = "Victory Plate"
	local subCM_NameY = "subcoat_gladiatorM"
	local subCF_Name = "Victory Plate Dress"
	local subCF_NameY = "subcoat_gladiatorF"
	local subW_Name = "Tetsubo"
	local subW_NameY = "gladiator_99w"
	local subS_Name = "Dragon's Skull"
	local subS_NameY = "gladiator_99s"
	...
	-- Sacrifices
		...
		-- Weapon
		local subW_ItemA = "Willbreaker"
		local subW_ItemAy = "warrior_95w"
		...
		elseif (sI_Menu == "Weapon") then
			...
			elseif (subWeapon == 4) then
				if (player:hasItem(subW_ItemAy, 1, player.id) == true) then
					...
					player.quest["subitem_Weapon"] = player.quest["subitem_Weapon"] + 1
					player:dialogSeq({t, "Alright, just give me one moment..",
						sG_Weapon, "Tada! Here it is, your new shiny "..subW_Name.." and "..subS_Name..".\n\nBe sure to take good care of it. If you need a replacement in the future, you can always return to get one.."}, 1)
					player:addItem(subW_NameY, 1, 0, player.id) --adds item to player, owner and engrave optional
					player:addItem(subS_NameY, 1, 0, player.id) --adds item to player, owner and engrave optional
