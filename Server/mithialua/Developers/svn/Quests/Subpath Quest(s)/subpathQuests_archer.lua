...
click = function(player, npc)
	...
	local sG_CoatM = {graphic = convertGraphic(2717, "item"), color = 12}
	local sG_CoatF = {graphic = convertGraphic(2718, "item"), color = 12}
	local sG_Weapon = {graphic = convertGraphic(2691, "item"), color = 11}

	local subCoat = player.quest["subitem_Coat"]
	local subWeapon = player.quest["subitem_Weapon"]

	local className = "Archer"

	local subCM_Name = "Ranger's Jerkin"
	local subCM_NameY = "subcoat_archerM"
	local subCF_Name = "Ranger's Jerkin Skirt"
	local subCF_NameY = "subcoat_archerF"
	local subW_Name = "Dragon's Longbow"
	local subW_NameY = "archer_99w"
