...
click = function(player, npc)
	...
	local sG_CoatM = {graphic = convertGraphic(4028, "item"), color = 0}
	local sG_CoatF = {graphic = convertGraphic(3623, "item"), color = 21}
	local sG_Weapon = {graphic = convertGraphic(2526, "item"), color = 12}

	local subCoat = player.quest["subitem_Coat"]
	local subWeapon = player.quest["subitem_Weapon"]

	local className = "Seer"

	local subCM_Name = "Prophet's Vestment"
	local subCM_NameY = "subcoat_seerM"
	local subCF_Name = "Prophet's Drapery"
	local subCF_NameY = "subcoat_seerF"
	local subW_Name = "Destiny's Fan"
	local subW_NameY = "seer_99w"
