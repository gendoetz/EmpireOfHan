...
on_attacked = function(mob, player)
	...
	local spoilchance = 0
	local coalChance = math.random(1,2)
	...
	if (block.id == mob.id) then
		if (pick ~= nil and (pick.id == 40083 or pick.id == 40084 or pick.id == 40085)) then
		...
		elseif (player.registry["miner"] >= 18000 and player.registry["miner"] < 50000) then
			...
			mob:addHealth(-10)
		elseif (player.registry["miner"] >= 50000) then
			...
			if (spoilchance == 1) then
				...
				if (coalChance == 1) then
					player:addItem("coal", 1)
					player:sendMinitext("You've found a chunk of coal along with the ore!")
				end
			end			
		else
...
--	end
	...
	if (fragChance <= fragGive) then
		player:addItem("cel_fragore", 1)
		player:sendMinitext("A celestial fragment falls from the deposit.")
	end
