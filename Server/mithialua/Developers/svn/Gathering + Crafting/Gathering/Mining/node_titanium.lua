...
on_attacked = function(mob, player)
	...
	if (block.id == mob.id) then
		...
		if (player.registry["miner"] < 124000) then
			spoilchance = 0
...
--	end
	...
	if (fragChance <= fragGive) then
		player:addItem("sol_fragore", 1)
		player:sendMinitext("A solar fragment falls from the deposit.")
	end	
