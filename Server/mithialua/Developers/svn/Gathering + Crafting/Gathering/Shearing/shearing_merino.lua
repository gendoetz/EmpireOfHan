...
on_attacked = function(mob, player)
	...
	if (block.id == mob.id) then
		if (pick ~= nil and (pick.id == 188 or pick.id == 40102 or pick.id == 40103 or pick.id == 40104)) then
			--player:flushDuration(1)
			--Calculate spoils based on mining skill 840
		if (player.registry["shearer"] >= 220 and player.registry["shearer"] < 840) then
		...
		elseif (player.registry["shearer"] >= 2200 and player.registry["shearer"] < 6400) then
			...
			mob:addHealth(-10)
		elseif (player.registry["shearer"] >= 6400) then
			spoilchance = math.random(2)
			mob:addHealth(-10)
		end
			if (spoilchance == 1) then
				player:addItem("fibre_merino", 1)
