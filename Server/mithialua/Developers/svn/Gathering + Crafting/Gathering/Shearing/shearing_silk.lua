...
on_attacked = function(mob, player)
	...
	if (block.id == mob.id) then
		if (pick ~= nil and (pick.id == 40103 or pick.id == 40104)) then
			--player:flushDuration(1)
			--Calculate spoils based on mining skill 840
		if (player.registry["shearer"] < 124000) then
			spoilchance = 0
...
mob:sendAnimationXY(193, mob.x, mob.y, 5)
end,
...
local newX = math.random(-1, 1)
local newY = math.random(-1, 1)
...
local mobCheck = mob:getObjectsInCell(mob.m, mob.startX + newX, mob.startY + newY, BL_MOB)
	...
	if (passCheck == 0 and #mobCheck == 0 and tileCheck == 8047) then
